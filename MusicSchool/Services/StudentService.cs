...
public class StudentService : IStudentService
{
    ...
    public async Task<ApiResult<IEnumerable<StudentResponse>>> GetAllCategoriesAsync()
    {
        ...
        return new ApiResult<IEnumerable<StudentResponse>>(HttpStatusCode.OK, students);
    }

    public async Task<ApiResult<StudentResponse>> GetStudentAsync(int id)
    {
        ...
        if (student == null)
        {
            return new ApiResult<StudentResponse>(HttpStatusCode.NotFound, message: null);
        }
        ...
        return new ApiResult<StudentResponse>(HttpStatusCode.OK, new StudentResponse(student.Id, $"{student.FirstName} {student.LastName}", student.DateOfBirth, instruments));
    }

    public async Task<ApiResult<Student>> UpdateInstrumentAsync(int id, [FromBody] UpdateStudentPut request)
    {
        var student = await _context.Student
            .SingleOrDefaultAsync(x => x.Id == id);
    ...
    public async Task<ApiResult<StudentResponse>> UpdateStudentInstrumentsAsync(int id, [FromBody] UpdateStudentInstrumentsPatch request)
    {
        var student = await _context.Student
                .Include(x => x.Instruments)
                .SingleOrDefaultAsync(x => x.Id == id);
       ...
       if (student == null)
       {
            return new ApiResult<StudentResponse>(HttpStatusCode.NotFound, "student not found");
       }
    ...
    public async Task<ApiResult<Student>> CreateStudentAsync([FromBody] CreateStudentRequest request)
    {
    ...
    public async Task<ApiResult<Student>> DeleteStudentAsync(int id)
    {
        var student = await _context.Student
        ...
        if (student == null)
        {
            return new ApiResult<Student>(HttpStatusCode.NotFound, $"Student: {id} not found");
        }
